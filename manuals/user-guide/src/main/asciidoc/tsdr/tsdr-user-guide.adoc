...
TSDR gets it's information from a variety of sources, which can be secured in different ways.

** OpenFlow Security
*** The OpenFlow plugin, which can be configured with Transport Layer Security (TLS).

** SNMP Security
*** The SNMP version3 has security support. However, since ODL SNMP plugin does not support version3, we (TSDR) will not have security support at this moment.
