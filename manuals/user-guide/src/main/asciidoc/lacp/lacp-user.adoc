...
2. Flow should be added manually on OpenFlow switches. Since LACP does not create flows and its create the Link Aggregation Group (LAG) and add the ports to LAG membership.
====
...
===== Prerequisites
Legacy switches should be enabled with LACP mode and configured with a long timeout.
