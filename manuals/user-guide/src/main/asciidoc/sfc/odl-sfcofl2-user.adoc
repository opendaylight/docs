...
Its possible for the SFF to also act as a classifier. This table maps subscriber
...
For SF nodes that dont support NSH tunneling, the IP header DSCP field is
...
This table is only populated when PktIn packets are received from the switch
...
Services Path, RPS ID) and NSI (Network Services Index, next hop) fields
...
use VLAN for the SFF-SF. RSP Paths 3 and 4 are symmetric NSH paths. Since its 
