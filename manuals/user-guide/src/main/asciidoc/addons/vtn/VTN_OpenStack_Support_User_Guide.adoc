...
. Server Setup
image::vtn_devstack_setup.png["LAB Setup" ,width= 500]
...
.Note: 
  .Please ensure that the eth0 interface is the default route and it is able to reach the ODL_IP_ADDRESS. 
  .The entries for eth1 are not mandatory, If not set, we may have to manually do "ifup eth1" after the stacking is complete to activate the interface.
