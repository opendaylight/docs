...
Next sections will describe how to configure BGP manually or using RESTCONF.
...
MIGHT NOT BE NEEDED: depending on your BGP router, you might need a switch from
linkstate attribute type 99 to 29. Check with your router vendor. Switch the
...
to false; to use IANA assigned type set the value to true or remove (true by
default)
...
==== Configure Connection Attributes (all in milliseconds) - OPTIONAL ====
...
- *min-sleep* - Minimum sleep time in between reconnect tries
- *max-sleep* - Maximum sleep time in between reconnect tries
- *sleep-factor* - Power factor of the sleep time between reconnect tries
- *connect-time* - How long we should wait for the TCP connect attempt, overrides default connection timeout dictated by TCP retransmits
...
BGP speaker needs to register all peers that can be connected to it (meaning if
...
In order to get routes advertised to other peers, you have to also configure the peers, like it's described in section BGP Peer //TODO: internal jump to section?
...
<ipv4-routes xmlns="urn:opendaylight:params:xml:ns:yang:bgp-rib">
 <ipv4-route>
  <prefix>200.20.160.="urn:opendaylight:params:xml:ns:yang:controller:bgp:rib:impl">Ibgp</peer-role>1/32</prefix>
  <attributes>
   <ipv4-next-hop>
