...
For the Beryllium release, multiple, simultaneous renderers will not be supported.
Instead either the VTN or GBP renderer feature can be installed, but
not both.

For the Beryllium release, the only actions supported are "ALLOW" and
...
For the Beryllium release there is no default implementation of a renderer,
