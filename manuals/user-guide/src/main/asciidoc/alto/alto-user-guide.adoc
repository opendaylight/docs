...
In the Beryllium release, ALTO uses the YANG model described in
...
[source,bash]
karaf > feature install alto-release
...
Use `curl` to request for ALTO services. Below is an example of using ECS service. With the example input below you can query the cost between the endpoints.

[source,bash]
HOST_IP=localhost                   # IP address of the controller
BASE_URL=$HOST_IP:8080/controller/nb/v2/alto
SERVICE_PATH=endpointcost/lookup
curl -l -H "Content-type: application/alto-endpointcostparams+json" -X POST -d `cat example-input-ecs.json` $BASE_URL/$SERVICE_PATH -v

[source,json]
include::example-ecs-input.json[]

==== For Internal users

Either call the RPC directly or process the results with corresponding `ResourceTranslator`.
