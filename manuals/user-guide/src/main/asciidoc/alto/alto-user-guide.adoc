...
Please make sure that you install the *Lithium* version of ALTO. You could
download the latest pre-built file from https://www.opendaylight.org/downloads[here],
or download the historical releases from https://www.opendaylight.org/software/release-archives[here].

If you want to build ALTO from the source code, you should clone ALTO from
https://git.opendaylight.org/gerrit/#/admin/projects/alto[Gerrit]
or https://github.com/opendaylight/alto[GitHub],
and *checkout to Lithium branch*.

=== Overview

The ALTO project provides support for _Application Layer Traffic
Optimization_ services defined in https://tools.ietf.org/html/rfc7285[RFC
7285].
...
[[manage-data,Managing Data with RESTCONF]]
...
Below is the content of example-input.json.

[source,json]
include::example-input.json[]

*note*: There need to be no space and newline in your JSON file.
Please delete all spaces and newlines if you use the example above.
...
==== Managing Data with RestConf API Explorer

Use the command below in Karaf to enable the RestConf API Explorer.
And you could access this API Explorer from http://<controller-ip>:8181/apidoc/explorer/index.html

[source,bash]
------------------------
karaf > feature:install odl-restconf-all
------------------------

You can find alto-service in the API Explorer and click on it.

Find **/config/alto-service:resources/network-maps/{resourcce-id}/**
and then click on the one which begins with PUT.

Fill in the value of **resource-id** and the **(config)network-map**.
Please make sure that the value of resource-id is equal to the value of **"alto-service:resource-id"**
in the (config)network-map and the input of the (config)network-map is similar to the JSON file showed
in **Managing Data with RESTCONF**. After that, just click on **try it out!**.

For example, if you use the JSON file showed in <<manage-data, Managing Data with RESTCONF>>,
you need to fill the value of **resource-id** with **test_odl**.

.Restconf API Explorer For Put a networkmap
image::alto-put-networkmap.png["Put networkmap image",width=500]

If you receive the response below, you have put the network map into the ALTO successfully!

.Upload the Networkmap successfully
image::alto-put-success.png["Success image",width=500]

If you want to see the network map that you have put in, you can click on **/config/alto-service:resources/network-maps/{resourcce-id}/**
which begins with **GET**. Type in your resource-id and click on **try it out!**.

Alternatively, you can also use curl command mentioned in <<manage-data, Managing Data with RESTCONF>> to get the result.

.Check the network map with apidoc
image::alto-networkmap-result.png["Result image",width=500]

==== Managing Data with `alto-manager`
...
Currently `network-map`, `cost-map` and `endpoint-property-map` are supported.
Also the resource files used in `alto-manager` follow the RFC7285-compatible format
instead of RESTCONF format.
...
Below is the content of example-rfc7285-networkmap.json.

[source,json]
include::example-rfc7285-networkmap.json[]

Use the following command to see the network map you have just put.

[source,bash]
------------------------
HOST_IP=localhost                   # IP address of the controller
BASE_URL=$HOST_IP:8080/controller/nb/v2/alto
SERVICE_PATH=networkmap
RESOURCE_ID=test_odl
curl -X GET -H "Content-type: application/networkmap+json" \
            http://$BASE_URL/$SERVICE_PATH/$RESOURCE_ID
------------------------
