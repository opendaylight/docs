...
== Content
- Developer Guide: List and Description of UNI Manager APIs
...
=== Functionality
The UNI manager plugin enables the creation of Ethernet Virtual Connections (EVC) as defined in by the Metro Ethernet Forum (MEF). An EVC provides a simulated Ethernet connection among LANS existing at different geographical locations. This version of the plugin is limited to connecting two LANS.

As defined by MEF, each location to be connected must have a User Network Interface, (UNI) which is a device that connect the user LAN to the EVC providers network.

In Beryllium release, UNI and EVC are really tied up to Open_vSwitch, leveraging the OVSDB project: creating a UNI will end up creating an OVSDB node with an _ovsbr0_ bridge, interface and port. While creating a UNI, based on the MEF requirement, one can specify a desired QoS; this leverage the QoS and Queue tables from the OVS database. (see documentation bellow for full details). +
...
Finally, within this release, UniMgr is more a Proof Of Concept than a framework to be used in production. Previous demonstration where made using Raspberry Pis, having a low NIC bandwith, thus the speed as defined in the API is actually mapped as follow:
