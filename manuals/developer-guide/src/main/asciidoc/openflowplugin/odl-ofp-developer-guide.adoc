...
=== Technical Architecture
include::odl-ofp-sequence-diagrams.adoc[]

//// * OpenDaylight_OpenFlow_Plugin::Config_Subsystem[ConfigSubsystem]

//// * OpenDaylight_OpenFlow_Plugin::Message_Spy[Message spy - internal counter statistics]
...
* https://git.opendaylight.org/gerrit/gitweb?p=openflowplugin.git;a=tree;f=distribution/base;hb=HEAD[local plugin base distro]

* https://git.opendaylight.org/gerrit/gitweb?p=integration.git;a=tree;f=distributions/base;hb=HEAD[integration base distro]

* https://git.opendaylight.org/gerrit/gitweb?p=ovsdb.git;a=tree;f=distribution/opendaylight;hb=HEAD[ovsdb opendaylight distro]
