...
* _resources_ contains useful scripts, How-To's, demos and other resources.

* _schemas_ contains the OVSDB schemas that are implemented in ODL.

* _southbound_ contains the plugin for converting from the OVSDB protocol to mdsal and vice-versa.

* _utils_ contains a collection of utilities for using the openflowplugin, southbound, neutron and other helper methods.
