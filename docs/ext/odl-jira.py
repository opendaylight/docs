...
class JiraFixedIssuesDirective(Directive):
    ...
    def run(self):
        ...
        table = [
            '.. list-table:: Issues resolved in versions %s through %s (`JIRA <https://jira.opendaylight.org/issues/?jql=%s>`_)' % (from_ver, to_ver, quote(query)),
            '   :class: datatable',
            '   :header-rows: 1',
            '   :widths: auto',
            '',
...
class JiraKnownIssuesDirective(Directive):
    ...
    def run(self):
        ...
        # FIXME: this is not quite nice: can we emit the table markup directly
        table = [
            '.. list-table:: Issues affecting versions %s through %s (`JIRA <https://jira.opendaylight.org/issues/?jql=%s>`_)' % (from_ver, to_ver, quote(query)),
            '   :class: datatable',
            '   :header-rows: 1',
            '   :widths: auto',
            '',
