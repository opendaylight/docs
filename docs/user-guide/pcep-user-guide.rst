...
The PCE operates on a network graph, built from the TED, in order to compute paths based on the path computation request issued by the PCC.
...
The PCE's knowledge of the availability of network resources (TED) and active LSPs awareness (LSP-DB) allows to perform automated application-driven network operations:
