...
The PCE's knowledge of the availability of network resources (TED) and active LSPs awareness (LSP-DB) allows to perform automatized application-driven network operations:
...
In addition, offers LSP management functionality, for Active Stateful PCE - the cornerstone for majority of PCE-enabled SDN solutions.
...
26. **address** - A PCC IP address.
27. **password** - MD5 authentication phrase.

.. warning:: The PCE (*pcep-topology-provider*) configuration is going to be changed in Carbon release.
...
In PCEP-SR draft version 6, SR-ERO/SR-RRO subobjects IANA code points change was proposed.
...
16. **path-setup-type** - Set *1* for SR-TE LSP

21. **ipv4-node-id** - The SR-ERO subobject represents *IPv4 Node ID* NAI.

22. **m-flag** - The SID value represents an MPLS label.

23. **sid** - The Segment Identifier.
...
3. **name** - The LSP name. If this parameter is omitted, re-synchronization is requested for all PCC's LSPs.
