...
the entry point in the SFC pipeline will be table 82 (SFC_TRANSPORT_CLASSIFIER_TABLE), and from that point, packet processing
will be similar to the :ref:`OpenFlow pipeline <sfc-user-guide-sfc-of-pipeline>`, just with another set of specific tables for
the SFC service.
This picture shows the SFC pipeline after service integration with Genius:

.. figure:: ./images/sfc/LSFF_pipeline.png
   :alt: sfc lsff pipeline

   SFC pipeline for Logical SFF
