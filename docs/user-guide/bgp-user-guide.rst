...
**URL:** ``/operational/bgp-rib:bgp-rib/rib/bgp-example/peer/bgp:%2F%2F127.0.0.2``
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
The OpenDaylight BGP implementation also supports routes injection via *applciation peer*.
Such peer has its own programmable RIB, which can be modified by user.
This concept allows user to originate new routes or override existing and advertise them all connected remote peers.
...
Now the injected route appears in application's peer RIBs and in local speaker's Loc-RIB:
...
9. Injected route is present in application peer's Adj-RIB-In.
