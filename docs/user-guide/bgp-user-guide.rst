...
The OpenDaylight BGP implementation provides a set of APIs (described below), that give he its operational state refreshed periodically, by default every 5 seconds.
...
@line 8: True if the peer supports graceful restart.

@line 9: True if we support graceful restart.
...
@line 5: At this moment the cost for count path under effect-rib-in is to high. Therefore the value is the same than total prefixes.
