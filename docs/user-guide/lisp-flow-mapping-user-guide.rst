...
Overlay Network Engine (ONE) is a FD.io project that enables programmable
...
1.  Pull the VPP codes anonymously using:
    ::
...
6.  Now, install and run OpenDaylight Boron release on the VM. Please
    ...
    with the ``lisp.elpPolicy`` to be replce.
    ::
...
8.  In this example, VPP doesn't make any southbound map registers to ODL.
    ...
    <https://git.opendaylight.org/gerrit/gitweb?p=lispflowmapping.git;a=tree;f=resources/tutorial/FD_io;hb=refs/heads/master>`__.
    Even though there is no south bound registration for mapping to OpenDayLight, using
