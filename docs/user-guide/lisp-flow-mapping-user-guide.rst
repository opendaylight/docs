...
build a simple/toy IP4 LISP overlay on an Ubuntu host using namespaces
and af_packet interfaces. All configuration files used in the tutorials
can be found `here <https://gerrit.fd.io/r/gitweb?p=one.git;a=tree;f=tutorial;hb=HEAD>`__.
...
9.  In this example, VPP doesn't make any south bound map registers to ODL.
    So, we add the mappings directly from North bound. For that, we need
    to add the mappings from to ODL via RESTCONF API.
