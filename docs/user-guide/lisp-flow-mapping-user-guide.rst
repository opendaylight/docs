...
Both has different approaches to create the overlay but ultimately does the
same job.
...
Creating a simple LISP overlay with VPP/ONE
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
-  **The Postman Chrome App**: the most convenient way to follow along
   ...
   top, choosing *Download from link* and then entering the following
   URL:
   ``https://gerrit.fd.io/r/gitweb?p=one.git;a=blob;f=tutorial/odl-one-config.json.postman_collection;hb=HEAD``.
   Alternatively, you can save the file on your machine, or if you have
   the repository checked out, you can import from there. You will need
   to create a new Postman Environment and define some variables within:
   ``controllerHost`` set to the hostname or IP address of the machine
...
5.  In case there is any error from ``vagrant up``, try ``vargant ssh``. if
    ...
    1. Pull the codes in ``/`` directory. So, the codes will be in ``/vpp``.
...
6.  By now, you should have a Ubuntu VM with VPP repository in ``/vpp``
    ...
    Success in the build create the binary in ``/vpp/build-root/install-vpp_lite_debug-native/vpp/bin``
...
8.  Now, we will add conf files for vpp. We will have two VPP running in
    different network namespaces. We will will teo conf files and we will
...
10. In this example, VPP doesn't do South bound map register. So, we
    control the mapping directly from North bound. For that, we need
