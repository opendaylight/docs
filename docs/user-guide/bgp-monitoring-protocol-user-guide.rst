...
==================================
This guide contains information on how to use OpenDayliht BGP Monitoring Protocol (BMP) plugin.
The user should learn about BMP basic concepts, supported capabilities, configuration and operations.
...
Overview
--------
This section provides high-level overview of the BMP, OpenDaylight implementation and BMP usage in SDN era.
...
^^^^^^^^^^^^^^^^^^^^^^^
The BGP Monitoring Protocol serves to monitor BGP sessions.
...
BMP messages are send by monitored router only, monitoring station supposed to collect and process data received over BMP.
