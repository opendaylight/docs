...
* Managed Snapshot Integrated (MSI) projects have now become devoid of active contributors. Their continued presence
  ...
  still lists ``Aluminium GA`` as unreleased even though that SimRel, including ``jsonrpc`` of that time, shipped
  5 years ago and has been desupported since September 2021. This tracking (creating and releasing releases in Jira)
  can only be done by active committers on each individual project as our governance explicitly precludes all other
  community members from doing so as part of project sovereignty.
