...
- Export ${RELEASE} and ${BUILDNUM} with current release name and build number.
       ...
       .. code-block:: bash
           ...
           export RELEASE-UPSTREAM=$(echo ${RELEASE} | cut -d'-' -f 1)
           export BUILDNUM=55
...
- Checkout autorelease and switch to release branch eg stable/carbon **(Release Engineering Team)**
    ...
    .. code-block:: bash
        ...
        git checkout -b stable/${RELEASE-UPSTREAM,,} origin/stable/${RELEASE-UPSTREAM,,}
        git submodule update --init
        git submodule foreach git checkout stable/${RELEASE-UPSTREAM,,} origin/stable/${RELEASE-UPSTREAM,,}
...
- Download patches (*.bundle files and taglist.log.gz) from log server.
    ...
    .. code-block:: bash
        ...
        wget https://logs.opendaylight.org/releng/jenkins092/autorelease-release-${RELEASE-UPSTREAM,,}/${BUILDNUM}/archives/all-bundles.tar.gz
        wget https://logs.opendaylight.org/releng/jenkins092/autorelease-release-${RELEASE-UPSTREAM,,}/${BUILDNUM}/archives/patches/taglist.log.gz
...
- Run the following commands for every project in the release, to apply patches to each project directory.
    ...
    .. code-block:: bash
        ...
        git push gerrit release/${RELEASE-UPSTREAM,,}
...
- Tag autorelease too
    ...
    .. code-block:: bash
        ...
        git tag -asm "OpenDaylight ${RELEASE} release" release/${RELEASE-UPSTREAM,,}
        git push gerrit release/${RELEASE-UPSTREAM,,}
