...
an email to the TSC mailing list. To facilitate quickly acting on problems
identified during TSC meetings, proxies must be committers to the project they
are proxying for. A single person can represent any number of projects.

Representatives will make the follwing entry into the meeting minutes to record
their presence:
...
Artifacts will released with the release job and checked with the
validate-autorelease job to ensure they load without breaking the distribution.
