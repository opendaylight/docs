...
the first release candidate (RC) is created. Since bumping and integrating with
upstream takes time, it is strongly recommended for Self-Managed projects to start
...
service release (e.g. by formal release if they want to be in SR1).
...
In order to be included in the formal (or service) release final distribution,
...
the first release candidate (RC) is created. Projects should only be added to the
final distribution pom.xml after they have suceesfully published artifacts using
upstream SNAPSHOTs. See `SNAPSHOT Versions by Release`_.
