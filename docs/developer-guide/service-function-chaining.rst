...
This section explains the changes made to the SFC classifier, thus enabling it 
to attach to Logical SFFs. 
...
reach table 82 (SFC classifier table), comming from table 0 (table managed by 
...
1. **Push the NSH header, along w/ its metadata (required within the SFC pipeline)**
   ...
   Features the specified ACL matches as match criteria (alongside the priority)
   ,and push NSH / NSH metadata Actions into the Action list.

2. **Advance the SFC pipeline**

   Forward the traffic to the first Service Function in the RSP. This steers 
   packets into the SFC domain, and how it is done depends on wheather the 
   classifier is co-located with the first service function the specified RSP. 
   ...
   interface. 
   These actions are then appended the the push NSH / push NSH metadata Action 
   list, and written in an Apply-Actions Instruction into the datastore.
