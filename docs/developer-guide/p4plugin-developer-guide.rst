   ...
   -  **add-device**
      ...
      -  Add a P4 device, users need to provide node id, device id, gRPC server address, config file
         ...
         the exception.
             ...
             1.If node id or P4 target address(device id and gRPC sever address) already exist.
...
-  p4plugin/core/api/src/main/yang/p4plugin-runtime.yang
   ...
   -  **modify-table-entry**
      ...
      -  Modify an existing entry to a specific device, the same input as the add method.
...
-  p4plugin/core/api/src/main/yang/p4plugin-cluster.yang
   ...
   -  **set-election-id**
      ...
      -  Set the election id, as for the meaning of the election id, it is not in this scope, please
         ...
         When a new election id is set, it will send master arbitration update message to all devices
         it connected.
