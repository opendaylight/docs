...
| If this file does not exist, then simply run karaf from ``<karaf_folder>/bin/``
  and type these commands:
| ``feature:install odl-topoprocessing-framework``
| ``logout``
| Now mentioned config file should be on its place so edit it.

| Also you have to add dependency required to test "inventory" topologies.
| In ``<topoprocessing_directory>/features/pom.xml``
  add ``<openflowplugin.version>0.3.0-SNAPSHOT</openflowplugin.version>``
