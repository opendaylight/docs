...
1. Update the odlparent version from 14.1.6 to 14.3.1. There should
 ...
 .. code-block:: shell
  ...
  bump-odl-version odlparent 14.1.6 14.3.1

2. Update the direct yangtools version references from 14.0.20 to 15.0.2,
 ...
 .. code-block:: shell
  ...
  bump-odl-version yangtools 14.0.20 15.0.2
...
4. Update the Controller version from 12.0.3 to 13.0.2. There should not be
 ...
 .. code-block:: shell
  ...
  bump-odl-version controller 12.0.3 13.0.2

5. Update the InfraUtils version from 7.1.9 to 7.1.12. There should not be
   any reference to **org.opendaylight.infrautils**, except for 7.1.12.

 .. code-block:: shell

  bump-odl-version infrautils 7.1.9 7.1.12
...
8. Update the IETF version from 1.0.2 to 2.0.2. There should not be
   any reference to **org.opendaylight.ietf**, except for 2.0.2.

 .. code-block:: shell

  bump-odl-version ietf 1.0.2 2.0.2

9. Update the gNMI version from 2.0.0 to 3.0.0. There should not be
...
<https://github.com/opendaylight/odlparent/compare/v14.1.6...v14.3.1>`_
...
See `YANGTOOLS-1780 <https://lf-opendaylight.atlassian.net/browse/YANGTOOLS-1780>`__ for more details.
...
Add this parameter into initialization of these classes.
For example:
...
Depending on your implementation, add these dependencies to resolve ``IllegalStateException: No YangXPathParserFactory found`` or
...
`YANGTOOLS-1699 <https://lf-opendaylight.atlassian.net/browse/YANGTOOLS-1699>`__
and
`YANGTOOLS-1707 <https://lf-opendaylight.atlassian.net/browse/YANGTOOLS-1707>`__
...
See `YANGTOOLS-1707 <https://lf-opendaylight.atlassian.net/browse/YANGTOOLS-1707>`__ for more details.
...
An example of the refactor can be found in `commit 6597252 <https://github.com/opendaylight/gnmi/commit/659725225488248fb2a4e2eaf1e8751c767cbff4>`__.

Update DatabindContext import
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

The ``DatabindContext`` was migrated to ``yangtools`` in `commit 2265500 <https://github.com/opendaylight/yangtools/commit/2265500b3adbc7945f8ebd355e27b80e919bb046>`__.
...
* Please take into account that since Vanadium YANG models related artifacts have been moved from
  ``MD-SAL`` to the new ``ietf`` project. Update your dependencies accordingly. The new group-id is ``org.opendaylight.ietf``.
* See the `project proposal <https://lf-opendaylight.atlassian.net/wiki/spaces/ODL/pages/282525740/IETF+Liaison+Project>`__.
...
There is no longer a need to add a default domain into ``StoreBuilder``.
For more info, follow: `AAA-295 <https://lf-opendaylight.atlassian.net/browse/AAA-295>`__.
...
.. note::
   ...
   included as a MRI Project starting with the Chromium release.
...
Due to changes made in the latest Chromium gNMI release, we will no longer be
...
New usage:
  ...
  .. code-block:: java
    ...
    final var gnmiProvider = new GnmiSouthboundProvider(
