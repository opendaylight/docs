...
JDK 21 Version
^^^^^^^^^^^^^^
2025.09 Vanadium, requires Java 21, both during compile-time and run-time.
...
This improvement allows us to use above example instead of the below one, improving type safety and avoiding NPE:
...
<<<<<<< HEAD   (1cab73 Update downloads page for Titanium SR3 release)
* Update your YANG models imports if necessary, for example see:
