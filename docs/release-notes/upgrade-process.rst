...
DataSchemaNode's isConfigration() is tri-state
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
A number of entrypoints, most notably to XML and JSON codecs, now take an ``EffectiveStatementInference`` instead of
...
MD-SAL Impacts
--------------
This MD-SAL release contrains a completely rewritten Java Binding generator.

The implementation now performs a multi-pass generation as opposed to a memoized single-pass generation approach
taken by the previous implementatation. Multiple passes allow for proper name allocation policies, with conflicts
...
In this release these compatibility methods are no longer genenerated, as detailed in
...
In this release these compatilibity methods are no longer generated, as detailed in
