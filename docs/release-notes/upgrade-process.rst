...
`RFC7950 <https://tools.ietf.org/html/rfc7950#section-6.4>`_.
...
`RFC7950 <https://tools.ietf.org/html/rfc7950#section-9.9.2>`_, augmented
...
a ``YangParserFactory`` reference properly injected and pass it to the
...
created at dynamically by MD-SAL runtime no longer discern between
...
`MD-SAL issue <https://jira.opendaylight.org/browse/MDSAL-449>`_.


YANG lists with key an system ordering result in Map
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Type mapping of simple keyed lists has changed to better reflect
they nature. Previously a YANG declaration
...
`MD-SAL issue <https://jira.opendaylight.org/browse/MDSAL-434>`_.
...
Artifacts are advertized through controller-artifacts
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
// FIXME: this did not work correctly for some time I think
...
is desupported and discouraged. While some services be advertized with this
