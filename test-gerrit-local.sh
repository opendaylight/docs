...
get_latest_patchset() {
    ...
    # Query Gerrit REST API to get change info
    local api_url="https://${gerrit_host}/gerrit/changes/${change_number}"
    local response
    if command -v curl >/dev/null 2>&1; then
        response=$(curl -s "${api_url}" 2>/dev/null | sed '1d' 2>/dev/null) # Remove first line (XSSI protection)
        if [[ -n "$response" ]] && echo "$response" | grep -q '"_number"'; then
            # Extract current_revision and find its patchset number
            local current_revision=$(echo "$response" | grep -o '"current_revision":"[^"]*"' | cut -d'"' -f4)
            if [[ -n "$current_revision" ]]; then
                local patchset=$(echo "$response" | grep -A5 "\"$current_revision\"" | grep -o '"_number":[0-9]*' | cut -d':' -f2)
                [[ -n "$patchset" ]] && echo "$patchset" && return 0
            fi
        fi
    fi
